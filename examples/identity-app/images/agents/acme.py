...
class AcmeAgent(DemoAgent):
    def __init__(
        self, http_port: int, admin_port: int, service_addr: str, **kwargs):
        super().__init__(
            "Acme Agent",
            http_port,
            admin_port,
        ...
        self.seed = os.environ['ENDORSER_SEED'] if 'ENDORSER_SEED' in os.environ else 'random'
...
async def main(start_port: int, service_addr: str, show_timing: bool = False):
    ...
    try:
        ...
        agent = AcmeAgent(
            start_port, start_port + 1, service_addr=service_addr, genesis_data=genesis, timing=show_timing
        )
