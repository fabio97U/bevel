...
async function GetByteCode(numberOfIterations, dirname, entrypoint, contractName) {
  ...
  const compilationResult = solc.compile(stringifiedJson);  // compiling the smart contract using the main entrypoint file
  var output = JSON.parse(compilationResult); 
