...
/*******************************************************
* Definition of flow to package items into containers
* @returns something
*******************************************************/

@InitiatingFlow
@StartableByRPC
class PackageItem (val productTrackingID: UUID, val containerTrackingID: UUID ) : FlowLogic<UUID>() {
    companion object {
    ...
    override fun call():UUID {
        ...
        //Generate TrackableState output with new containerID
        val trackOutput = trackInput.state.data.withNewTracking(containerID = containerTrackingID)
        ...
        val txBuilder = TransactionBuilder(notary)        
                ...
                .addCommand(SupplyChainContract.Commands.PackContainer(), trackOutput.participants.map { it.owningKey })
...
@InitiatedBy(PackageItem::class)
open class PackageItemResponder(val counterpartySession: FlowSession) : FlowLogic<SignedTransaction>() {
