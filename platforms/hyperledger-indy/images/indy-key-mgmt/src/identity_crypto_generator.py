...
class VaultUploader:
    ...
    def upload(self, data):
        ...
        provider = list(data.to_dict().keys())[0]
        ...
        if not provider_exists:
            ...
            self.send_data(url=url, headers=headers, data={"type": "kv"})

        organisation = '{}/{}'.format(data.vault_path.replace('.', '/'), data.name)
        organisation_url = '{}/v1/{}'.format(self.address, organisation)
        organisation_exists, _ = self.read_data(url=organisation_url, headers=headers)
    ...
    def process_data(self, vault_paths, headers):
        for path in vault_paths:
            for key, value in path.items():
                ...
                exists_path, vault_data = self.read_data(url=url, headers=headers)
                if exists_path:
                    data = {**vault_data, **value}
                else:
                    data = value
                self.send_data(url=url, data=data, headers=headers)
...
class IdentityCreator:
    ...
    def process(cls):
        ...
        identity_crypto_generator = IdentityCryptoGenerator(
            identity_name=args.identity_name, vault_path=args.vault_path
        )
        vault = VaultUploader(address=args.vault_address)

        identity = identity_crypto_generator.generate()
